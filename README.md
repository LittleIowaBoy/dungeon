...
- **T23 — Frozen biome CHILLED status** ✅: new `CHILLED` constant, `_DEFAULTS` entry (3s/0.5x), and `speed_multiplier()` hook in `status_effects.py`; blue tint visual in `player_visual_rules.py`; `TrapVentLane.apply_player_pressure` applies CHILLED when `vent_chilled_duration_ms > 0`; new template fields `trap_vent_chilled_duration_ms` (default 0, ice=3000) through the full data pipeline; 3 new tests
- **T24 — Sunken biome surge event** ✅: new template fields `trap_surge_interval_ms` / `trap_surge_duration_ms` (default 0, sunken=12000/1500) through the full data pipeline; `room.py` `_update_trap_surge()` method tracks surge state in the controller; `surge_active` overrides lane suppression in all three `_active()` methods; HUD shows `SURGE INCOMING` 2s before and `SURGE ACTIVE` during; 7 new tests
- **T25 — Spike panel hazard** ✅: new `TrapSpikePanel` sprite class (floor panel toggling on a fixed cycle, surge override, warn visual); `mixed_lanes` rotation extended to 4-way including `spike_panel`; `_trap_spike_panel_configs()` builder in `room.py`; sprite factory in `dungeon.py`; 4 new tests
