...
class GameRuntimeTests(unittest.TestCase):
    ...
    def test_apply_room_objective_update_spawns_enemy_instances(self):
        game = rpg.Game.__new__(rpg.Game)
        game.dungeon = SimpleNamespace(enemy_group=_DummyGroup(), chest_group=_DummyGroup())
    ...
    def test_apply_room_objective_update_marks_looted_and_upgrades_existing_chests(self):
        ...
        chest_b = _DummyChest()
        game = rpg.Game.__new__(rpg.Game)
        game.dungeon = SimpleNamespace(enemy_group=_DummyGroup(), chest_group=_DummyGroup([chest_a, chest_b]))
    ...
    def test_apply_room_objective_update_restores_reclaimable_chests(self):
        ...
        chest.looted = True
        game = rpg.Game.__new__(rpg.Game)
        game.dungeon = SimpleNamespace(enemy_group=_DummyGroup(), chest_group=_DummyGroup([chest]))
    ...
    def test_room_test_spawn_position_avoids_center_portal_tile(self):
        center_col = rpg.ROOM_COLS // 2
        center_row = rpg.ROOM_ROWS // 2

        def tile_at(col, row):
            if abs(col - center_col) <= 1 and abs(row - center_row) <= 1:
                return rpg.PORTAL
            return "floor"

        game = rpg.Game.__new__(rpg.Game)
        game.dungeon = SimpleNamespace(current_room=SimpleNamespace(tile_at=tile_at))

        spawn_x, spawn_y = rpg.Game._room_test_spawn_position(game)

        self.assertEqual(
            (spawn_x, spawn_y),
            (
                center_col * rpg.TILE_SIZE + rpg.TILE_SIZE // 2,
                (center_row + 2) * rpg.TILE_SIZE + rpg.TILE_SIZE // 2,
            ),
        )